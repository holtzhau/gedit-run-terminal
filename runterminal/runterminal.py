...
class TerminalWindowHelper(object):
    def __init__(self, plugin, window):
        ...
        if not self.gconf_client.dir_exists(self.gconfdir):
            self.gconf_client.add_dir(self.gconfdir, gconf.CLIENT_PRELOAD_NONE)
            self.gconf_client.set_int(self.gconfdir+"/rightpanel_position", window.get_size()[0]-200)
            self.gconf_client.set_bool(self.gconfdir+"/panel_at_right", False)
        self._window = window
        ...
        bottom.add_item(self._panel, _("Run Terminal"), image)
        self.vbox = bottom.get_parent()
        self.hbox = self.vbox.get_parent()
...
#        self._insert_menu()
    ...
    def toggle_item(self, menu_item):
        if self.panel_at_right:
            ...
            self.panel_at_right = False
        else:
            self.move_panel_right()
            self.panel_at_right = True
        self.gconf_client.set_bool(self.gconfdir+"/panel_at_right", self.panel_at_right)
    
    def move_panel_right(self):
        bottom = self._window.get_bottom_panel()        
        self.vbox.reparent(self.hpane)
        bottom.reparent(self.hpane)
    ...
    def move_panel_bottom(self):
        bottom = self._window.get_bottom_panel()
        v = bottom.get_parent()
        self.hbox.remove(self.hpane)
        self.vbox.reparent(self.hbox)
        bottom.reparent(self.vbox)
        self.vbox.add2(bottom)
